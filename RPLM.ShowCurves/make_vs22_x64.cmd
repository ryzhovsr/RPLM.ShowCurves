...
"C:\Main\Programs\Microsoft Visual Studio\Common7\IDE\CommonExtensions\Microsoft\TeamFoundation\Team Explorer\tf" get %CD% /recursive
)

SET cmake_path="%cd%/../CMake/"
IF EXIST %cmake_path% (
	SET build_path="%cd%/../../builds/vs17/x64/RPLM.CAD.ShowCurves"
...
IF %onlygenerateproject% == 1 (
	...
	cmake -S"." -G"Visual Studio 17 2022" -B%build_path% -A"x64" -DCMAKE_INSTALL_PREFIX=%install_path% -DCMAKE_CONFIGURATION_TYPES="%config_types%"
	goto:EOF
)
